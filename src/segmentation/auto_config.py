...
* **Anisotropy** (``anisotropy = z_step / xy_pixel``) — read from the TIFF's
  OME-XML or ImageJ metadata via :mod:`segmentation.io.metadata`. This is a physical
...
test, mirroring the style of ``segmentation.cellpose``.
"""
...
from .io.metadata import Spacing, resolve_spacing
