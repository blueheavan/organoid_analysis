...
class ZStack:
    ...
    def z_count(self) -> int:
        return self.volume.shape[-3]

    def display_spacing(self) -> tuple[str, str, str]:
        """Return (x, y, z) spacing strings for the UI, '?' when unknown."""
        return tuple(
            f"{v:.4g} µm" if v is not None else "?"
            for v in (self.spacing.x, self.spacing.y, self.spacing.z)
        )
