...
The vendored ``vtk.js`` UMD bundle (``vendor/vtk.js``) is inlined into the
...
from .presets import DEFAULT_LUTS, PRESETS, Lut
...
_VENDOR = Path(__file__).parent / "vendor" / "vtk.js"
...
def validate_spacing(spacing_um: Sequence[float]) -> tuple[float, float, float]:
    if spacing_um is None or len(spacing_um) != 3:
        raise ValueError(
            f"spacing_um must have exactly 3 values (x, y, z), got {spacing_um!r}"
        )
    s = tuple(float(v) for v in spacing_um)
    if any(not math.isfinite(v) or v <= 0 for v in s):
...
def render_viewer_html(
    ...
    *,
    ...
    if inline_vtkjs:
        ...
        script = (
            '<script src="/ui/vtk_viewer/vendor/vtk.js"></script>\n'
            + "<script>"
            + viewer_js(spec_json)
            + "</script>"
        )
