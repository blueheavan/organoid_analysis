...
def _render_current_session_runner() -> None:
    ...
    from segmentation.cellpose import list_saved_results, restore_saved_result
    ...
    if st.button("Run multilevel 3D analysis", type="primary", key="multilevel_run_current"):
        ...
        if spacing is None:
            ...
            return
        try:
            from analysis.multilevel3d import analyze_multilevel_3d
            from analysis.multilevel3d.export import export_results
            from segmentation.cellpose import read_stack
...
def _load_exported_labels(root: Path) -> tuple[dict[str, np.ndarray], tuple[float, float, float]] | None:
    ...
    from analysis.io import read_tiff
...
def _render_multilevel_geometry(root: Path, tables: dict[str, pd.DataFrame]) -> None:
    ...
    from ui.vtk_viewer.core import ChannelConfig, st_volume_viewer
