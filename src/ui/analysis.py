...
from segmentation.paths import DATA_DIR
...
def load_excel_dataset(
    ...
    """Load an Excel cell-statistics sheet into a DataFrame."""
...
def detect_metadata_columns(df: pd.DataFrame) -> list[str]:
    """Return non-feature columns (sample / organisation metadata)."""
    return [col for col in df.columns if not col.startswith(FEATURE_PREFIXES)]
